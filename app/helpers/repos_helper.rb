module ReposHelper
  def status_info(repo)
    ...
    when "synced"
      ...
      if !remote_status[:errors].empty?
        short_status = remote_status[:errors].first[:short_description]
        long_status = remote_status[:errors].first[:long_description]
      elsif remote_status[:mismatched_checksums].empty? && remote_status[:missing_packages].empty?
        short_status = status[:state].humanize.to_s
