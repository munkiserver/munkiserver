  ...
  def users_who_can_read(controller_name)
    ...
    principals.each do |principal|
      if principal.is_a? User
        ...
        raise UserGroupException, "While retrieving users who can read #{controller_name} from a unit (#{self}), found a principal that was not a User or UserGroup record!"
      end
    end
    users.uniq(&:id)
  end
