...
class PackageBranch < ActiveRecord::Base
  ...
  def new_version?(unit = nil)
    if version_tracker.nil? || version_tracker.version.nil?
      ...
      if version_string.present?
        ...
        raise Exception, "No package found with package branch of #{self} inside #{unit} unit"
      end
    end
  end
  ...
  def new_version_shared?(unit)
    ...
    latest_package = latest_where_unit(unit)
    # Packages from this branch, which are shared, not part of unit
    latest_shared_package = all_packages.shared.where("unit_id != #{unit.id}").order("version desc").limit(1).first
  ...
  def self.available_updates(unit = nil)
    ...
    if unit.present?
      ...
      packages_with_updates = latest_packages.delete_if { |p| p.nil? || !p.new_version? }
    else
  ...
  def self.unit_member(unit_member)
    Package.unit(unit_member.unit).environments(unit_member.environments).map(&:package_branch).uniq
  end
  ...
  def self.unit_and_environment(unit, environment)
    Package.unit(unit).environment(environment).uniq_by(&:package_branch_id)
  end
  ...
  def obsolete?
    packages.empty? && install_items.empty? && uninstall_items.empty? && managed_update_items.empty? && optional_install_items.empty? && require_items.empty? && update_for_items.empty?
  end
