module IsAManifest
  ...
  module ClassMethods
    ...
    def find_for_show(unit, s)
      ...
      record = self.where(id: s).first if s =~ /^\d+$/
      # Find by id-name
      match = s.match(/^(\d+)([-_]{1})(.+)$/)
  ...
  def precedent_items(item_name, exclusion_items_hash)
    ...
    third_item_hash = (self.is_a?(Computer) && self.computer_group.present?) ? create_item_hash(self.computer_group.send("precedent_#{item_name}")) : {}

    aux_items = third_item_hash.merge(second_item_hash)
    aux_items.delete_if { |k, _v| exclusion_items_hash[k].present? }
  ...
  def create_item_array(item_method, using_precedent_items = true)
    ...
    self.send(method.to_s).each do |item|
    ...
    unless list.nil?
      list.each do |item|
        ...
        if item.class == Package
          # ...a specific package
          self.send(assoc.to_s).build(package_id: item.id, package_branch_id: item.package_branch.id)
        elsif item.class == PackageBranch
          # ...the latest package from a package branch
          self.send(assoc.to_s).build(package_branch_id: item.id)
        end
      end
    end
  end
