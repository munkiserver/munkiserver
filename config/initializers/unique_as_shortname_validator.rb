...
class UniqueAsShortnameValidator < ActiveModel::EachValidator
  def validate_each(record, attribute, value)
    ...
    single_result_is_not_this_record = (matching_records.count == 1 && matching_records.first.id != record.id)
    if single_result_is_not_this_record || matching_records.count > 1
