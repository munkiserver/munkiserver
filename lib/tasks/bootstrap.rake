...
namespace :bootstrap do
  ...
  task computer_models: :environment do
    ...
    a += [{ name: "PowerMac4,4", icon_path: "#{path}/com.apple.emac.png" },
  ...
  task :unit, [:name] => :environment do |_t, args|
    if Unit.count == 0
      Rake::Task["bootstrap:environments"].invoke if Environment.count == 0
      name = args.name
      name ||= "Default"
      u = Unit.new(name: name, description: "Created by bootstrap")
      puts "Default unit failed to save: " + u.errors.inspect unless u.save
    end
  end

  desc "Create root user"
  task root_user: :environment do |_t, _args|
    if User.where(username: "root").first.blank?
      ...
      puts "Failed to save root user: " + u.errors.inspect unless u.save
    end
  end
  ...
  task create_assets_directory: :environment do
    ...
    `mkdir #{assets_dir}` unless File.exist?(assets_dir)
  end
