namespace :chore do
  ...
  task :cleanup_missing_manifests, [:days_kept] => [:environment] do |_t, args|
  ...
  task :cleanup_old_managed_install_reports, [:days_kept] => [:environment] do |_t, args|
    args.with_defaults(days_kept: 30)
  ...
  task :validate_models, [:validate_models] => :environment do |_t, _args|
  ...
  task :generate_shortnames, [:generate_shortnames] => :environment do
    ...
    records.each do |record|
      next unless record.shortname.blank?
  ...
  task destroy_unused_package_files: :environment do
    Dir["#{Munki::Application::PACKAGE_DIR}/*.*"].each do |path|
  ...
  task conform_restart_actions: :environment do
    ...
    conformed_values.each_pair do |old_value, new_value|
